...
from django.db.models.signals import pre_delete, post_migrate, pre_save
...
def ensure_roles_exist(sender, **kwargs):
    ...
    Runs only for apps that have the 'requires_role_creation' attribute set to True in their AppConfig.
    """
    app_config = apps.get_app_config(sender.label)

    # Check if the app_config has the custom attribute 'requires_role_creation'
